...
namespace AppContext {
  constexpr char standaloneName[] = "botpp";
...
std::unique_ptr<dotname::IBot> uniqueLib;
...
int handlesArguments (int argc, const char* argv[]) {
  try {
    ...
    if (!result.count ("omit")) {
      // uniqueLib = std::make_unique<dotname::IBot> ();
      uniqueLib = std::make_unique<dotname::IBot> (AppContext::assetsPath);
    } else {
